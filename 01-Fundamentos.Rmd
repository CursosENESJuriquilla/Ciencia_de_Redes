...
## Grafos: matemáticas de las redes.
...
2) Determine cuántos trayectorias edge-disjoint existen entre $s$ y $t$.
...
(a) ¿Hay un vuelo directo entre Indianápolis y Fairbanks, Alaska (FAI)? Un vuelo directo es aquel sin escalas.

(b) Si quisiera volar desde Indianápolis a Fairbanks, Alaska, ¿cuál sería el itinerario con el menor número de vuelos?

(c) ¿Es posible viajar desde cualquier aeropuerto de EE. UU. a cualquier otro, posiblemente con vuelos de conexión? En otras palabras, ¿existe una ruta en la red entre cada par de aeropuertos posibles?


#### Ejercicio 2: Conectividad y robustez en redes sociales animales

En este ejercicio trabajaremos con redes reales del repositorio **Animal Social Network Repository (ASNR)**, disponibles en formato `GraphML`.

- [Sitio ASNR](https://bansallab.github.io/asnr/data.html)

- [GitHub con las redes en formato graphml](https://github.com/bansallab/asnr/tree/master/Networks)

- [Archivo de metadatos](https://github.com/bansallab/asnr/blob/master/Network_summary_masterfile.csv)

Cada red representa interacciones sociales entre individuos de una especie animal. El objetivo es analizar propiedades estructurales básicas de estas redes y relacionarlas con los conceptos vistos en clase, como grado promedio, diámetro, caminos disjuntos por aristas y conjuntos de corte mínimos.

**Instrucciones generales**

1. Elige **dos redes** correspondientes a especies distintas del repositorio.
2. Carga ambas redes en `Python` usando `networkx`.
3. Responde cada inciso con apoyo de código, cálculos e interpretación.
4. Justifica tus respuestas usando las definiciones y resultados vistos en clase.

**Parte 1.** Propiedades básicas de la red

Para cada una de las dos redes elegidas, calcula lo siguiente:

- número de nodos \(n\),
- número de aristas \(m\),
- grado promedio,
- diámetro de la red (si la red no es conexa, trabaja con su componente conexa más grande e indícalo explícitamente).

Responde lo siguiente:

a. ¿Cuál de las dos redes tiene mayor grado promedio?  
b. ¿Cuál tiene mayor diámetro?  
c. ¿Qué podría sugerir esto sobre la organización de la red?


**Parte 2.** Caminos disjuntos por aristas y corte mínimo

Para cada red, selecciona un par de nodos \(u,v\). Puedes elegirlos al azar o elegir dos nodos que pertenezcan a la componente conexa más grande.

Calcula:

- el número de **caminos disjuntos por aristas** entre \(u\) y \(v\),
- el tamaño de un **conjunto de corte mínimo por aristas** entre \(u\) y \(v\).

Responde lo siguiente: 

a. ¿Coiniden ambos valores?  
b. Explica por qué este resultado está relacionado con las ideas vistas en clase sobre conectividad y corte mínimo.  
c. ¿Qué interpretación puedes dar en términos de la robustez de la conexión entre esos dos individuos?

**Parte 3.** Eliminación de un nodo importante

Para una de las dos redes, identifica el nodo de mayor grado.

Después:

1. elimina ese nodo de la red;
2. calcula nuevamente el número de componentes conexas;
3. calcula el diámetro de la componente conexa más grande de la nueva red.

Responde lo siguiente:

a. ¿La red se fragmentó al eliminar ese nodo?  
b. ¿Cambió el diámetro de manera importante?  
c. ¿Qué sugiere esto sobre el papel estructural de los nodos de alto grado?

**Parte 4.** Interpretación final

Escribe un breve párrafo comparando ambas redes.

Tu discusión debe incluir:

- cuál parece más conectada,
- cuál parece más vulnerable a la eliminación de enlaces o nodos,
- cómo se relacionan estas observaciones con los conceptos de grado, diámetro y corte mínimo.


#### Ejercicio 3: Comparación de redes de personajes en dos películas
...
1. Elige **dos películas diferentes** del sitio Moviegalaxies.
2. Descarga los archivos de red en formato **JSON**. (Debes investigar como leer estos archivos en `Networkx`)
3. Carga ambas redes en `Python` usando `networkx`.
4. Responde cada inciso con apoyo de código, cálculos e interpretación.
...
a. ¿Cuál de las dos redes tiene **más personajes**?  

b. ¿Cuál tiene **mayor grado promedio**?  

c. ¿Cuál tiene **mayor diámetro**?  

d. ¿Qué sugiere el diámetro sobre la distancia social entre personajes en cada historia?

**Parte 2.** Personajes centrales

Para cada red:

1. calcula el grado de todos los nodos,
2. identifica el personaje con **mayor grado**.

Responde lo siguiente:

a. ¿Quién es el personaje más conectado en cada película?  

b. ¿Cuántos personajes diferentes interactúan con él o ella?  

c. ¿Coincide este personaje con quien considerarías el protagonista de la historia?
...
a. ¿Coinciden ambos valores?  

b. ¿Qué significa esto en términos de la **conectividad entre esos personajes**?  

c. ¿Cuántas interacciones tendrían que desaparecer para separarlos en la red?
...
1. elimina el nodo de **mayor grado**,
2. calcula nuevamente:
   - número de componentes conexas,
   - diámetro de la componente conexa más grande.

Responde lo siguiente:

a. ¿La red se fragmenta al eliminar ese personaje?  

b. ¿En cuál de las dos redes el efecto es mayor?  

c. ¿Qué indica esto sobre la **robustez estructural** de la red?
...
nx.draw(K, with_labels=True, node_color='lightblue')
plt.show()
```
...
En nuestro ejemplo de la red de Karate:

```{python}
c_k = nx.degree_centrality(K)
sorted((v, f"{c:0.2f}") for v, c in c_k.items())
```
...
Por default $\beta=1.0$ y $\alpha= 0.1$
...
#### Centralidad de Entre Aristas (Betweenness)
...
Hay diferentes consideraciones que se pueden hacer en esta función.

- `normalized=False`: NO escala los valores, da los conteos "crudos"

- `endpoints=False`: No cuenta los nodos extremos del camino, es decir, solo cuenta nodos intermedios.
...
Funciones principales en NetworkX

| Función                      | Qué hace                           |
| ---------------------------- | ---------------------------------- |
| `find_cliques(G)`            | Encuentra cliques maximales        |
| `enumerate_all_cliques(G)`   | Encuentra todas las cliques        |
| `node_clique_number(G)`      | Tamaño de la mayor clique por nodo |
| `number_of_cliques(G)`       | Cuántas cliques contiene cada nodo |
| `max_weight_clique(G)`       | Clique máxima con pesos            |
| `k_clique_communities(G, k)` | Comunidades basadas en cliques     |
...
Recuerda:
- Una clique maximal no puede agrandarse.
- No tiene que ser la más grande globalmente.
...
- Tamaño de la clique máxima: Si queremos encontrar el tamaño de la clique más grande:
...
- `number_of_cliques(G)`: Cuenta cuántas **cliques maximales** contiene cada nodo.
...
Incluye:
- cliques de tamaño 1 (nodos)
- cliques de tamaño 2 (aristas)
- cliques de tamaño 3, 4, etc.Todas las cliques (no solo maximales):
...
print("Número de cliques maximales:", len(maximal))
print("Tamaño de la clique máxima:", len(largest))
print("Una clique máxima:", largest)
```


- **Visualización de una clique encontrada**: Podemos resaltar una clique máxima dentro del grafo.
...
La función devuelve:
1. la clique con mayor suma de pesos
2. el peso total de esa clique
...
list(k_clique_communities(K, k=3))
```


*Ejercicio 1*: Construye `nx.path_graph(5)`.

1. Lista todas las cliques.
2. Lista las cliques maximales.
3. ¿Cuál es el tamaño de la clique máxima?

*Ejercicio 2*: Construye un grafo con aristas:
`(1,2), (2,3), (1,3), (3,4), (4,5), (3,5)`

1. Encuentra las cliques maximales.
2. ¿Cuántos triángulos hay?
3. ¿Qué nodo pertenece a más cliques maximales?

*Ejercicio 3*: En el grafo `nx.cycle_graph(6)`:

1. ¿Qué cliques maximales aparecen?
2. ¿Hay alguna clique de tamaño 3?
3. Explica por qué.

*Ejercicio 4*: Usa `nx.karate_club_graph()`:

1. Encuentra una clique máxima.
2. Calcula cuántas cliques maximales hay.
3. Identifica los nodos con mayor `number_of_cliques`.

*Ejercicio 5*: Explica con tus palabras la diferencia entre:
- clique
- clique maximal
- clique máxima
...
- *Conectividad por aristas*: Mide cuántas **aristas** hay que quitar como mínimo para desconectar una red.

- *Conectividad por nodos*: Mide cuántos **nodos** hay que quitar como mínimo para desconectar una red.

- `k_edge_components`: Un k-edge-component es un conjunto maximal de nodos tal que cualquier par está conectado por al menos `k` caminos independientes por aristas.

- `k_components`: Un k-component es un subgrafo maximal cuya conectividad por nodos es al menos `k`.
...
dibuja(G, pos, "Ejemplo 1")
```


```{python}
print("k-edge-components con k=1:")
print(list(nx.k_edge_components(G, k=1)))

print("\nk-edge-components con k=2:")
print(list(nx.k_edge_components(G, k=2)))

print("\nedge_connectivity global:", nx.edge_connectivity(G))
print("node_connectivity global:", nx.node_connectivity(G))
print("\nk-components:")
...
dibuja(H, posH, "Dos triángulos compartiendo un vértice")
```


```{python}
for k in [1, 2]:
    print(f"k={k} -> {list(nx.k_edge_components(H, k=k))}")

print("\nk-components:")
print(nx.k_components(H))

## Cut vertex
print("\nPuntos de articulación:")
print(list(nx.articulation_points(H)))
...
dibuja(C, posC, "Ciclo de 5 nodos")
```


```{python}
print("edge_connectivity:", nx.edge_connectivity(C))
print("node_connectivity:", nx.node_connectivity(C))
print("2-edge-components:", list(nx.k_edge_components(C, k=2)))
print("k-components:", nx.k_components(C))
```

En un ciclo la conectividad sube a 2: no basta quitar una sola arista o un solo nodo para romperlo de la misma forma que un camino.
...
```{python}
P = nx.path_graph(5)
posP = {i:(i,0) for i in P.nodes()}
dibuja(P, posP, "Camino de 5 nodos")
```

```{python}
print("edge_connectivity:", nx.edge_connectivity(P))
print("node_connectivity:", nx.node_connectivity(P))
print("k-edge-components k=1:", list(nx.k_edge_components(P, k=1)))
print("k-edge-components k=2:", list(nx.k_edge_components(P, k=2)))
print("k-components:", nx.k_components(P))
```


*Ejemplo 5*: barbell graph (dos gráficas completas conectadas con una trayectoria)

```{python}
B  = nx.barbell_graph(5, 1)
posB = nx.spring_layout(B, seed=5)
dibuja(B, posB, "Barbell graph")
```


```{python}
print("edge_connectivity:", nx.edge_connectivity(B))
print("node_connectivity:", nx.node_connectivity(B))
print("2-edge-components:", list(nx.k_edge_components(B, k=2)))
print("k-components:", nx.k_components(B))
print("Puentes:", list(nx.bridges(B)))
print("Puntos de articulación:", list(nx.articulation_points(B)))
```
...
- `nx.k_edge_components(G, k)`
- `G`: grafo de NetworkX
- `k`: entero positivo, nivel de conectividad por aristas
...
print("Nodos:", K.number_of_nodes())
print("Aristas:", K.number_of_edges())
print("edge_connectivity:", nx.edge_connectivity(K))
print("node_connectivity:", nx.node_connectivity(K))
print("k-components:", nx.k_components(K))
```


```{python}
posK = nx.spring_layout(K, seed=10)
dibuja(K, posK, "Karate Club Graph")
```
...
1. Dibuja la red.
2. Calcula `list(nx.k_edge_components(G, k=2))`.
3. Calcula `nx.k_components(G)`.
...
G.add_edges_from([(1,2), (2,3), (1,3), (3,4)])
pos = {1:(0,1), 2:(1,1), 3:(0.5,0), 4:(1.8,0)}
dibuja(G, pos, "Triángulo con una cola")
```
...
print("Triángulos que pasan por 3:", nx.triangles(G, 3))
print("Triángulos que pasan por 4:", nx.triangles(G, 4))
```

- `nx.all_triangles(G, nbunch=None)`
...
for v in G.nodes():
    print(f"nodo {v}: grado={G.degree(v)}, clustering={nx.clustering(G, v):.3f}")
```

El clustering local mide qué proporción de pares de vecinos de un nodo también están conectados entre sí.

- `nx.average_clustering(G, ..., count_zeros=True)`

```{python}
print("Promedio incluyendo ceros:", nx.average_clustering(G))
print("Promedio sin contar ceros:", nx.average_clustering(G, count_zeros=False))
```
...
`average_clustering` y `transitivity` no son exactamente lo mismo:
...
print("triangles:", nx.triangles(C))
print("all_triangles:", list(nx.all_triangles(C)))
print("clustering:", nx.clustering(C))
print("average_clustering:", nx.average_clustering(C))
print("transitivity:", nx.transitivity(C))
```

- Comparación con una clique completa

```{python}
K4 = nx.complete_graph(4)
posK4 = nx.circular_layout(K4)
dibuja(K4, posK4, "Clique completa K4")

print("triangles:", nx.triangles(K4))
print("all_triangles:", list(nx.all_triangles(K4)))
print("clustering:", nx.clustering(K4))
print("average_clustering:", nx.average_clustering(K4))
print("transitivity:", nx.transitivity(K4))
```


- Dos triángulos compartiendo un nodo

```{python}
H = nx.Graph()
H.add_edges_from([(1,2), (2,3), (1,3), (3,4), (4,5), (3,5)])
posH = {1:(0,1), 2:(1,1), 3:(0.5,0), 4:(1.8,1), 5:(2.3,0)}
dibuja(H, posH, "Dos triángulos compartiendo un nodo")

print("triangles:", nx.triangles(H))
print("all_triangles:", list(nx.all_triangles(H)))
print("clustering:", nx.clustering(H))
print("average_clustering:", nx.average_clustering(H))
print("transitivity:", nx.transitivity(H))
```


- `nx.square_clustering(G, nodes=None)`: puede detectar estructura cuadrada incluso cuando no hay triángulos.

```{python}
B = nx.cycle_graph(4)
posB = nx.circular_layout(B)
dibuja(B, posB, "Ciclo de 4 nodos")

print("triangles:", nx.triangles(B))
print("clustering:", nx.clustering(B))
print("square_clustering:", nx.square_clustering(B))
```

- `nx.generalized_degree(G, nodes=None)`: El grado generalizado refina el grado usual según cuántos triángulos comparte cada arista incidente.

```{python}
gd = nx.generalized_degree(H)
print(gd)
for v, d in gd.items():
    print(f"nodo {v}: {d}")
```
...
dibuja(W, posW, "Red con pesos")

print("Clustering sin pesos:")
print(nx.clustering(W))

print("\nClustering con pesos:")
print(nx.clustering(W, weight='weight'))

print("\nPromedio sin pesos:", nx.average_clustering(W))
print("Promedio con pesos:", nx.average_clustering(W, weight='weight'))
```
...
```{python}
K = nx.karate_club_graph()
print("Número de nodos:", K.number_of_nodes())
print("Número de aristas:", K.number_of_edges())
print("Average clustering:", nx.average_clustering(K))
print("Transitivity:", nx.transitivity(K))

tri = nx.triangles(K)
print("Algunos valores de triangles:")
for v in list(K.nodes())[:10]:
    print(v, tri[v])
```
...
1. Calcula `nx.triangles`.
2. Calcula `nx.clustering`.
3. Calcula `nx.average_clustering` y `nx.transitivity`.
4. Explica por qué.

*Ejercicio 2*: Construye `nx.cycle_graph(6)` y compáralo con `nx.cycle_graph(4)`.

1. ¿Qué pasa con `clustering`?
2. ¿Qué pasa con `square_clustering`?

*Ejercicio 3*: Construye una gráfica con aristas:
`(1,2), (2,3), (3,1), (3,4), (4,5), (5,3), (5,6)`.

1. Dibuja la red.
2. Encuentra todos los triángulos.
3. Calcula el clustering de cada nodo.
4. ¿Qué nodo participa en más triángulos?

*Ejercicio 4*: Usa `nx.complete_graph(5)`.

1. ¿Cuántos triángulos toca cada nodo?
2. ¿Cuál es el average clustering?
3. ¿Cuál es la transitividad?

*Ejercicio 5*: Construye una red con pesos donde haya un triángulo fuerte y otro débil.

1. Calcula clustering sin pesos.
2. Calcula clustering con pesos.
3. Interpreta la diferencia.




### Homofilia y Asortatividad en Redes
...
- `nx.degree_mixing_matrix`
- `nx.attribute_mixing_dict`
- `nx.degree_mixing_dict`
...
Ejemplos:
- por **grado**: nodos muy conectados con otros muy conectados;
- por **atributo categórico**: nodos del mismo grupo conectándose entre sí;
- por **atributo numérico**: nodos con valores parecidos conectándose entre sí.

Interpretación típica:
- valor positivo: mezcla entre similares;
- valor cercano a 0: sin patrón claro;
- valor negativo: mezcla entre diferentes.
...
- **Asortatividad por grado**: Estas funciones miden si nodos con cierto grado tienden a conectarse con nodos de grado parecido.
...
Los nodos más conectados (grado 3) se conectan más veces con nodos menos conectados (grado 2) que con nodos de su mismo grado.
...
dibuja(S, posS, "Star graph")

print("degree_assortativity_coefficient:", nx.degree_assortativity_coefficient(S))
print("degree_pearson_correlation_coefficient:", nx.degree_pearson_correlation_coefficient(S))
```

En una estrella, un nodo de grado alto se conecta con muchos nodos de grado bajo, así que suele aparecer asortatvidad negativa por grado.
...
dibuja(compl, posS, "Star graph")

print("degree_assortativity_coefficient:", nx.degree_assortativity_coefficient(compl))
print("degree_pearson_correlation_coefficient:", nx.degree_pearson_correlation_coefficient(compl))
```

La asortatividad mide correlación entre grados. Si todos los grados son iguales, no hay correlación que medir.
...
```{python}
print("degree_assortativity_coefficient:", nx.degree_assortativity_coefficient(G2))
print("degree_pearson_correlation_coefficient:", nx.degree_pearson_correlation_coefficient(G2))
```

- Asortatividad por atributo categórico:

```{python}
print("Assortativity por atributo 'group':", nx.attribute_assortativity_coefficient(G, "group"))
```

- Asortatividad por atributo numérico:

```{python}
print("Assortativity numérica para 'score':", nx.numeric_assortativity_coefficient(G, "score"))
```
...
- **Ejemplo**: Los conjuntos de datos `schoolfriends_vertices` y `schoolfriends_edgelist` del paquete `onadata` (o disponibles para descarga en línea) representan una red de amistades reportadas entre estudiantes de una escuela secundaria en Marsella, Francia, en 2013.
...
- *Amistad reportada*: cuando el estudiante con ID de origen (`from`) declara al estudiante con ID de destino (`to`) como su amigo.

- *Amistad en Facebook*: cuando existe una conexión confirmada en Facebook entre ambos estudiantes.

Ejemplo tomado de [Handbook of Graphs and Networks in People Analytics, Capítulo 8](https://ona-book.org/similarity.html)
...
1) Calcula el coeficiente de asortatividad para el género.

2) Calcula el coeficiente de asortatividad para el departamento.
...
1) Reporta los 5 personajes más importantes según cada medida.
...
## Estructura de Redes Empíricas - La estructura de las redes del mundo real
...
- Nombre: ego-Facebook
- Tipo: no dirigida
- Tamaño: 4,039 nodos, 88,234 aristas
- Interpretación: nodos = personas, aristas = amistad

**Descarga:** https://snap.stanford.edu/data/ego-Facebook.html
...
- Construye la distribución de grado $p_k$.
...
### Ejercicio 2: Red de colaboración científica (arXiv)
